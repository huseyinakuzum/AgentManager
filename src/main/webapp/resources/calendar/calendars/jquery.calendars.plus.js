	...
	$.extend($.calendars.baseCalendar.prototype, {
		...
		formatDate: function(format, date, settings) {
			...
			for (var iFormat = 0; iFormat < format.length; iFormat++) {
				if (literal) {
					...
					switch (format.charAt(iFormat)) {
						...
						case 'M': output += formatNumber('M', date.month(), 1); break;
						case 'MM': output += formatNumber('MM', date.month(), 2); break;
						case 'MMM': output += monthNamesShort[date.month() - this.minMonth]; break;
						case 'MMMM': output += monthNames[date.month() - this.minMonth]; break;
						case 'y':
						...
						case '$': output += (date.toJD() - this.UNIX_EPOCH) * this.SECS_PER_DAY * 1000; break;
			...
			if(format != 'yyyy-MM-dd')
				return convertDigits(output, this.local.digits);
			else
				return output;
		},
		...
		parseDate: function(format, value, settings) {
			...
			value = window.parseDigits(value, this.local.digits);
			if (value == null) {
			...
			for (var iFormat = 0; iFormat < format.length; iFormat++) {
				if (literal) {
					...
					switch (format.charAt(iFormat)) {
						...
						case 'MM': month = getNumber('M'); break;
						case 'MMM': month = getName('MMM', monthNamesShort, monthNamesShort); break;
						case 'MMMM': month = getName('MMMM', monthNames, monthNames); break;
						case 'y':
							var iSave = iFormat;
