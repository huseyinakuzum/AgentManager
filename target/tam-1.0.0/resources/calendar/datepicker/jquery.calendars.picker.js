...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).calendarsPicker({minDate: 0, maxDate: '+1m +1w'}) */
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {
				picker: '<div class="datepicker ' +
				'{popup:start} datepicker-dropdown dropdown-menu datepicker-orient-left datepicker-orient-top {popup:end} {inline:start} datepicker-inline{inline:end}" style="display:block;">' +
				'<div class="datepicker-days" style="display:block;">' +
				'{months}',
				monthRow: '<table class="table-condensed"><div class="ui-datepicker-row-break">{months}</div>',
				month: '<thead><tr>{link:prev}<th class="datepicker-switch" colspan="5">{monthHeader:MM yyyy}</th>{link:next}</tr>' +
				'<tr>{weekHeader}</tr></thead><tbody>{weeks}</tbody></table></div>',
				weekHeader: '<tr>{days}</tr>',
				dayHeader: '<th>{day}</th>',
				week: '<tr>{days}</tr>',
				day: '{day}',
				monthSelector: '.datepicker-days',
				daySelector: 'td',
		...
		commands: {
			prev: {text: 'prevText', status: 'prevStatus', // Previous month
				...
				enabled: function(inst) {
					...
					return (!minDate || inst.drawDate.newDate().
						add(1 - inst.options.monthsToStep - inst.options.monthsOffset, 'MM').
						day(inst.options.calendar.minDay).add(-1, 'd').compareTo(minDate) !== -1); },
				date: function(inst) {
					return inst.drawDate.newDate().
						add(-inst.options.monthsToStep - inst.options.monthsOffset, 'MM').
			...
			prevJump: {text: 'prevJumpText', status: 'prevJumpStatus', // Previous year
				...
				enabled: function(inst) {
					var minDate = inst.curMinDate();
					return (!minDate || inst.drawDate.newDate().
						add(1 - inst.options.monthsToJump - inst.options.monthsOffset, 'MM').
						day(inst.options.calendar.minDay).add(-1, 'd').compareTo(minDate) !== -1); },
				date: function(inst) {
					return inst.drawDate.newDate().
						add(-inst.options.monthsToJump - inst.options.monthsOffset, 'MM').
						day(inst.options.calendar.minDay); },
				action: function(inst) {
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
		...
		defaultOptions: {
			...
			pickerClass: '',
			showOnFocus: true,
			showTrigger: '.datepicker-trigger:eq(0)',
			showAnim: '',
			showOptions: {},
			showSpeed: 'normal',
			popupContainer: '#research.modal .modal-content',
			alignment: 'bottom',
			fixedWeeks: false,
			firstDay: null,
			calculateWeek: null,
		...
		_yearClass: 'calendars-dow-', // Marker for day of week
		_monthClass: 'calendars-dow-', // Marker for day of week
		...
		_update: function(elem, hidden) {
			...
			if (!$.isEmptyObject(inst)) {
				...
				if (inst.inline) {
                    ...
                    if (mobile && $(inst.options.popupContainer).length)
                        inst.div.css("margin-top", (0-$(inst.options.popupContainer).offset().top-4) + "px").appendTo($(inst.options.popupContainer));
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		changeMonth: function(elem, offset) {
			var inst = this._getInst(elem);
			if (!$.isEmptyObject(inst)) {
				var date = inst.drawDate.newDate().add(offset, 'MM');
		...
		_generateContent: function(elem, inst) {
			...
			var addCommand = function(type, open, close, name, classes) {
				...
				picker = picker.replace(new RegExp('\\{' + type + ':' + name + '\\}', 'g'),
					...
					' class="'+ name + ' ' + inst.options.renderer.commandClass + ' ' +
			...
			for (var name in inst.options.commands) {
				...
				addCommand('link', 'th style="display: table-cell;"', 'th', name,
			...
			var self = this;
			picker.find(inst.options.renderer.daySelector + ":not(."+inst.options.renderer.disabledClass.replace(' ','.')+")").hover(
			
					function() {
						(inst.inline ? $(this).closest('.' + self._getMarker()) : inst.div).
							find(inst.options.renderer.daySelector).
			...
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < calendar.daysInWeek(); day++) {
					...
					days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
						...
						window.convertDigits((inst.options.showOtherMonths || drawDate.month() === month ?
						dateInfo.content || drawDate.day() : '&nbsp;').toString(), inst.options.calendar.local.digits) +
						('</td>'));
					drawDate.add(1, 'd');
					jd++;
				}
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
			...
			var selector = '<select class="calendars-month ' + this._monthYearClass +
			...
			if (yearRange === 'any') {
				selector = '<select class="calendars-year ' + this._monthYearClass + ' ' + this._anyYearClass +
					'" title="' + inst.options.yearStatus + '">' +
					'<option>' + convertDigits(year.toString(), inst.options.calendar.local.digits) + '</option></select>' +
				...
				selector = '<select class="calendars-year ' + this._monthYearClass +
					'" title="' + inst.options.yearStatus + '">';
				...
				var addYear = function(y) {
					if (y !== 0 || calendar.hasYearZero) {
						selector += '<option value="' +
							...
							convertDigits(y.toString(), inst.options.calendar.local.digits) + '</option>';
					}
				};
